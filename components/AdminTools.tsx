  ...
  return (
    <div className="mt-6 bg-stone-50 p-5 rounded-3xl border-2 border-stone-200 relative animate-in fade-in slide-in-from-top-4 shadow-inner">
      ...
      <div className="flex gap-2 mb-4">
        <button
          ...
          className={`flex items-center gap-2 px-4 py-2 rounded-full text-xs font-bold transition-all ${
            activeTab === 'email' ? 'bg-[#6F8F72] text-white shadow-md' : 'bg-white text-stone-500 hover:bg-stone-100 border border-stone-200'
          }`}
        >
          <Send size={14} /> 草擬回信
        </button>
        <button
          ...
          className={`flex items-center gap-2 px-4 py-2 rounded-full text-xs font-bold transition-all ${
            activeTab === 'plan' ? 'bg-[#6F8F72] text-white shadow-md' : 'bg-white text-stone-500 hover:bg-stone-100 border border-stone-200'
          }`}
        >
      ...
      <div className="min-h-[120px] bg-white rounded-2xl p-4 text-stone-700 text-sm whitespace-pre-wrap border-2 border-stone-200 font-medium leading-relaxed">
        {loading ? (
            <div className="flex flex-col items-center justify-center h-20 gap-2 text-[#6F8F72]">
      ...
      <div className="mt-4 flex justify-end">
        <button
            ...
            disabled={loading}
            className="flex items-center gap-2 px-6 py-2.5 bg-gradient-to-r from-[#bcc9bc] to-[#aab5aa] hover:from-[#aab5aa] hover:to-[#98a398] text-[#2d2d2d] rounded-full text-xs font-bold transition-all disabled:opacity-50 shadow-md shadow-[#bcc9bc]/20 hover:-translate-y-0.5 active:scale-95"
        >
